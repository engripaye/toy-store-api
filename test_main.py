...
def test_read_toy_by_id():
    # First create a toy
    new_toy = client.post("/toys/", json={"name": "Football WORLD CUP", "price": 190.99, "in_stock": True}).json()
    toy_id = new_toy["id"]
...
def test_update_toy():
    # create a toy
    toy = client.post("/toys/", json={"name": "Football EURO'S CUP", "price": 170.89, "in_stock": True}).json()
    toy_id = toy["id"]

    # update it
    response = client.put(
        f"/toys/{toy_id}",
        json={"name": "Football EURO'S CUP", "price": 170.89, "in_stock": True}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_delete_toy():
    # create a toy
    toy = client.post("/toys/", json={"name": "Football NATION'S CUP", "price": 150.65, "in_stock": True}).json()
    toy_id = toy["id"]
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["detail"] == "Toy deleted successfully"
